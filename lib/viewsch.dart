...
class ViewReply extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const ViewSchall(title: 'Scholarships'),
    );
  }
}

class ViewSchall extends StatefulWidget {
  const ViewSchall({super.key, required this.title});

  final String title;

  @override
  State<ViewSchall> createState() => _ViewReplyPageState();
}

class _ViewReplyPageState extends State<ViewSchall> {
  ...
  Future<void> viewreply() async {
    ...
    try {
      ...
      String url = '$urls/student_viewall_scholarship/';
  ...
  Future<void> viewreply2() async {
    ...
    try {
      ...
      String urls = sh.getString('url').toString();
      String lid = sh.getString('lid').toString();
      String url = '$urls/student_view_all_scholarships/';
      ...
      for (int i = 0; i < arr.length; i++) {
        id.add(arr[i]['id'].toString());
        ScholarShip_Name.add(arr[i]['ScholarShip_Name'].toString());
        Scholarship_Number.add(arr[i]['Scholarship_Number'].toString());
        Eligibility.add(arr[i]['Eligibility'].toString());
        Score.add(arr[i]['Score'].toString());
        Start_Date.add(arr[i]['Start_Date'].toString());
        End_Date.add(arr[i]['End_Date'].toString());
        Renewal_Date.add(arr[i]['Renewal_Date'].toString());
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      ElevatedButton(
                                        ...
                                        child: Text(applied_[index]==''?"Apply":'Applied'),
                                      ),
                                    ],
                                  ),
                                ),
